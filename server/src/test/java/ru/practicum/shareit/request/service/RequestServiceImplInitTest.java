...
class RequestServiceImplInitTest {
    ...
    void getItemsWithPagination() {
        ...
        Assertions.assertEquals(2, itemRequestWithItemDtos.size(), "Array size must be 2");
        for (ItemRequestWithItemDto itemRequestWithItemDto : itemRequestWithItemDtos) {
            if (itemRequestWithItemDto.getId().equals(itemRequest1.getId())) {
                Assertions.assertNotNull(itemRequestWithItemDto.getItems().get(0), "Item in itemsRequest is null");
            }
        }
    }
}